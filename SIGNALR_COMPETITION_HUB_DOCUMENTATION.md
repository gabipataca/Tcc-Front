...
// Para o estudante que submeteu (recebe após processamento assíncrono)
...
**⚠️ Nota Importante sobre Processamento**:
- A submissão é **processada de forma assíncrona** por um worker em background
...
interface Question {
  ...
  askedBy: string; // Nome do usuário
  isAnswered: boolean;
  answer?: Answer | null;
} | null // null se não houver competição ativa
```
...
connection.on('ReceiveQuestionCreationResponse', (question: Question | null) => {
  if (question) {
...
connection.on('ReceiveQuestionCreation', (question: Question) => {
...
interface Answer {
  ...
  answeredBy: string; // Nome do professor/admin
  answeredById: string; // UUID do professor/admin
  isPrivate: boolean;
} | null // null se não houver competição ativa
```
...
**Descrição**: Bloqueia a capacidade de um grupo enviar submissões na competição.
...
4. **Processamento Assíncrono**: `SendExerciseAttempt` usa uma fila para processamento assíncrono. A resposta pode não ser imediata.

5. **Reconexão Automática**: Recomenda-se configurar `.withAutomaticReconnect()` no cliente SignalR.

6. **Logs**: Todas as ações importantes são registradas (login, logout, bloqueios, etc.).

7. **Timezone**: Todos os DateTime são retornados em UTC (ISO 8601).

8. **UUIDs**: IDs de usuários e grupos são UUIDs (strings), IDs numéricos são para exercícios, competições, etc.
